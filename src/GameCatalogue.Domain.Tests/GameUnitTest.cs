...
namespace GameCatalogue.Domain.Tests
{
    public class GameUnitTest
    {
        ...
        [Fact(DisplayName = "Create Game With Short Producer Value")]
        public void CreateGame_ShortProducerValue_DomainExceptionShortProducer()
        {
            Action action = () => new Game(1, "Game Name", "Prod", "Game Action", 1, "Game image");
            action.Should().Throw<DomainExceptionValidation>().WithMessage("Invalid producer, too short, minimum 5 characters");
        }

        [Fact(DisplayName = "Create Game With Missing Producer Value")]
        public void CreateGame_MissingProducerValue_DomainExceptionRequiredProducer()
        {
            Action action = () => new Game(1, "Game Name", "", "Game Action", 1, "Game image");
            action.Should().Throw<DomainExceptionValidation>().WithMessage("Invalid producer. Producer is required");
        }

        [Fact(DisplayName = "Create Game With Null Producer Value")]
        public void CreateGame_WithNullProducerValue_DomainExceptionInvalidProducer()
        {
            Action action = () => new Game(1, "Game Name", null, "Game Action", 1, "Game image");
            action.Should().Throw<DomainExceptionValidation>();
        }
